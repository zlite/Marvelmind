...
void hedgehog_send_packet(byte address, byte packet_type, unsigned int id, byte data_size)
{
	...
	else frameSizeBeforeCRC = 4;

	hedgehog_set_crc16( hedgehog_serial_buf, frameSizeBeforeCRC );
...
bool check_packet_data_size( byte packet_type, unsigned int packet_id, byte size, byte wanted_size )
{
...
void process_stream_packet()
{
	if (hedgehog_packet_id == HEDGEHOG_POS_PACKET_ID)
	{
		...
		hedgehog_z = (int)(hedgehog_serial_buf[13]) + (int)(hedgehog_serial_buf[14]) << 8;// height of hedgehog, cm
...
void loop_hedgehog()
{
	...
	while( Serial.available() > 0 )
	{
		...
		switch(hedgehog_serial_buf_ofs)
		{
			...
			case 3:
			{
				...
				hedgehog_packet_id = hedgehog_serial_buf[2] + incoming_byte << 8;
			...
			{
				// data field size
				if ( !check_packet_data_size(PACKET_TYPE_STREAM_FROM_HEDGE, HEDGEHOG_POS_PACKET_ID, incoming_byte, 0x10) )
					continue;// incorrect hedgehog coordinates data size

				if ( !check_packet_data_size(PACKET_TYPE_WRITE_TO_DEVICE, MOVEMENT_PATH_PACKET_ID, incoming_byte, 0x0c) )
	...
	if (packet_received)
	{
		hedgehog_set_crc16(hedgehog_serial_buf, hedgehog_packet_size);// calculate CRC checksum of packet
